...
class ExceptionDefine1 {
	...
	// throws AgeInputException : 현 메소드에서 AgeInputException 예외가 발생해도 처리하지 않겠다는 의미로서 
	// 예외처리를 현 메소드를 호출한 곳을 넘기는 명령 - 예외처리는 try-catch문으로 해야 함
		...
		if (age < 0) {
			...
			throw ex;
			// ex에 들이었는 AgeInputException 예외가 발생했다고 JVM에게 알리는 역할
			// throw가 실행되면 JVM의 예외처리 매카니즘이 동작하게 됨
		}
		return age;
	}
}
